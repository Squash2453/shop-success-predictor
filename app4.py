...
# Header
st.markdown('<p class="main-header">🏪 Shop Type Recommendation System</p>', unsafe_allow_html=True)
...
if st.button("🔮 Get Recommendations"):
    with st.spinner("Analyzing location data..."):
        ...
        for i, (shop, p) in enumerate(zip(top3_classes, top3_probs)):
            ...
            st.markdown(f"""
                <div style="background-color: {color}; padding: 1rem; border-radius: 8px; margin-bottom: 0.8rem; color: white;">
                    <h3 style="margin: 0; color: white; font-size: 1.3rem;">{medal} {shop}</h3>
                    <p style="margin: 0.3rem 0 0.5rem 0; color: white; font-size: 1.1rem; font-weight: bold;">Success Rate: {p*100:.2f}%</p>
                    <div style="background-color: rgba(255,255,255,0.3); height: 8px; border-radius: 4px;">
                        <div style="background-color: white; height: 100%; width: {p*100}%; border-radius: 4px;"></div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
